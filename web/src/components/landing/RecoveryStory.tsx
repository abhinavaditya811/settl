...
const Kicker = styled.div`
  position:absolute; top:26px; left:50%; transform:translateX(-50%); z-index:4;
  display:flex; gap:14px; align-items:center; white-space:nowrap;
  font:500 10px var(--font-mono); letter-spacing:.14em; text-transform:uppercase; color:rgba(255,255,255,.5);
  .keys{padding:4px 8px;border-radius:999px;border:1px solid rgba(255,255,255,.14);color:rgba(255,255,255,.62);}
...
const RailItem = styled.button<{ $on: boolean; $past: boolean }>`
  position:relative; display:grid; grid-template-columns:16px 28px 1fr; gap:9px; align-items:center; padding:10px 0;
  color:${({ $on, $past }) => ($on ? "#fff" : $past ? "rgba(255,255,255,.55)" : "rgba(255,255,255,.28)")};
...
`;
const Art = styled.div`position:relative; min-height:590px; display:grid; place-items:center; @media(max-width:980px){order:2;min-height:390px;}`;
...
const Verb = styled(motion.div)<{ $color: string }>`font:600 clamp(44px,5vw,76px)/.86 var(--font-display);letter-spacing:-.065em;color:${({ $color }) => $color};margin-bottom:20px;`;
const Title = styled.h2`font:600 clamp(28px,3.2vw,42px)/1 var(--font-display);letter-spacing:-.045em;margin:0;`;
const Body = styled.p`font:400 14.5px/1.65 var(--font-body);color:rgba(255,255,255,.58);margin:16px 0 0;`;
const Reason = styled.div`margin-top:24px;padding-top:16px;border-top:1px solid rgba(255,255,255,.12);font:500 10px/1.5 var(--font-mono);letter-spacing:.04em;color:rgba(255,255,255,.45);span{color:${mint};}`;
...
const MobileFlow = styled.div`display:none;@media(max-width:980px){display:block;background:${dark};}`;
...
function Scene({ stage }: { stage: number }) {
  ...
  return (
    <Art>
      <Halo $color={s.fg} animate={{ scale: [1, 1.04, 1], rotate: stage * 12 }} transition={{ scale: { duration: 4, repeat: Infinity }, rotate: { duration: 0.7, ease } }} />
      <Artifact layoutId="recovery-invoice" transition={{ duration: 0.55, ease }} animate={{ rotate: stage === 0 ? -4 : stage === 5 ? 0 : stage % 2 ? 2 : -1 }}>
        <ArtTop><span>INV-012 · Atlas Mechanical</span><span className="state">{s.label}</span></ArtTop>
        <BaseAmount>$6,800</BaseAmount>
        <BaseMeta>B2B · 21 days overdue · payment verified</BaseMeta>
        <AnimatePresence mode="wait">
          <motion.div key={s.key} initial={{ opacity: 0, y: 18 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -12 }} transition={{ duration: 0.38, ease }}>
            {stage === 0 && (
              <>
            ...
            {stage === 1 && (
              <Branches>
                <div className="route">wait 3 days<div className="line" /></div>
                <div className="route on">firm email<div className="line" />selected</div>
                <div className="route">voice call<div className="line" /></div>
              </Branches>
            )}
            {stage === 2 && (
              <Message>
                ...
                <div className="link">{"{{payment_link}}"}</div>
              </Message>
            )}
            {stage === 3 && (
              <>
                <Scanner initial={{ top: "34%" }} animate={{ top: "91%" }} transition={{ duration: 1.5, repeat: Infinity, repeatDelay: 0.6, ease: "easeInOut" }} />
                <Checks>
                  <div className="check ok">✓ B2B verified</div>
                  <div className="check ok">✓ no dispute</div>
                  <div className="check ok">✓ frequency clear</div>
                  <div className="check ok">✓ tone in bounds</div>
                  <div className="check ok">✓ URL placeholder</div>
                  <div className="check">● first touch approval</div>
                </Checks>
              </>
            )}
            {stage === 4 && (
              <SendScene>
                <div className="approval"><span>✓ APPROVAL RECORDED</span><span>✓ GATE RE-RUN · PASSED</span></div>
                <div className="trail" />
                <motion.div className="mail" animate={{ x: [-70, 0, 70], opacity: [0, 1, 0] }} transition={{ duration: 2, repeat: Infinity, ease }}>
                  <LedgerIcon name="send" />
                </motion.div>
                <span className="delivered">DELIVERED · 10:42</span>
              </SendScene>
            )}
            {stage === 5 && (
              <Paid>
                <motion.div className="tick" initial={{ scale: 0 }} animate={{ scale: 1 }} transition={{ type: "spring", stiffness: 260 }}>
                  <LedgerIcon name="reconcile" />
                </motion.div>
                <div className="paid">Payment verified</div>
                <div className="sub">recovery loop closed · 0 future sends</div>
              </Paid>
            )}
          </motion.div>
        </AnimatePresence>
...
export default function RecoveryStory() {
  ...
  const go = (i: number, opts?: { hash?: boolean; sound?: boolean }) => {
    ...
    window.scrollTo({ top: top + distance * (next / 5), behavior: reduce ? "auto" : "smooth" });
    if (opts?.hash !== false) setLandingHash(`stage-${next + 1}`);
    if (opts?.sound !== false) playLandingTone(next === 3 ? "scan" : "stage");
  };

  useMotionValueEvent(scrollYProgress, "change", (v) => {
    const next = Math.min(5, Math.floor(v * 6));
    setStage(next);
    if (next !== lastTone.current && v > 0.02 && v < 0.98) {
      lastTone.current = next;
      playLandingTone(next === 3 ? "scan" : "stage");
      setLandingHash(`stage-${next + 1}`);
    }
  });
  ...
  useEffect(() => {
    const onStage = (e: Event) => {
      ...
      if (typeof index === "number") go(index, { hash: false, sound: true });
    };
    ...
    window.addEventListener("settl:stage", onStage);
    window.addEventListener("keydown", onKey);
    return () => {
      window.removeEventListener("settl:stage", onStage);
  ...
  const s = STAGES[stage];
  return (
    <StoryRoot id="recovery-story">
      <Story ref={ref}>
        <Pin $tone={s.tone}>
          <Noise />
          <Kicker>
            <span>watch one invoice move · overdue to paid</span>
            <span className="keys">keys 1–6</span>
          </Kicker>
          <AnimatePresence mode="wait">
            <StageNo key={s.n} initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}>{s.n}</StageNo>
          </AnimatePresence>
          <Layout>
            <Rail>
              {STAGES.map((item, i) => (
                <RailItem key={item.key} $on={i === stage} $past={i < stage} onClick={() => go(i)} aria-label={`Go to ${item.label}`}>
                  <span className="dot" /><span className="n">{item.n}</span><span className="label">{item.label}</span>
                </RailItem>
              ))}
            </Rail>
            <Scene stage={stage} />
            <Copy>
              <AnimatePresence mode="wait">
                <motion.div key={s.key} initial={{ opacity: 0, y: 24 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -18 }} transition={{ duration: 0.42, ease }}>
                  <Verb $color={s.fg}>{s.verb}</Verb>
                  <Title>{s.title}</Title>
                  <Body>{s.body}</Body>
                  <Reason><span>WHY THIS MOVE</span><br />{s.reason}</Reason>
                </motion.div>
              </AnimatePresence>
            </Copy>
          </Layout>
      ...
      <MobileFlow>
        {STAGES.map((item, index) => (
          <MobileChapter key={item.key} id={`stage-${index + 1}-mobile`} $tone={item.tone}>
